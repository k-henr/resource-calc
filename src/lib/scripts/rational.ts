...
export class Rational {
	...
	constructor(num: number, den: number = 1) {
		...
		if (Math.floor(num) !== num || Math.floor(den) !== den) {
			const maxDecimalLength = Math.max(numDecimals(Math.abs(num)), numDecimals(Math.abs(den)));
	...
	// red if unparsable
	public static fromInput(inputString: string, inputEl: HTMLInputElement | null): Rational | null {
	...
	public mul(v: Rational | number) {
		if (typeof v === 'number') {
			...
			return new Rational(this.numerator * v.numerator, this.denominator * v.denominator);
		}
	}

	public div(v2: Rational) {
		return new Rational(this.numerator * v2.denominator, this.denominator * v2.numerator);
	}
	...
	public pow(v2: Rational) {
		...
		// precision
		if (v2.denominator !== 1)
			throw new GraphError("There's currently no support for raising a number to a non-integer!");
	...
	public floor() {
		...
		for (const v of [approximateResult + 1, approximateResult, approximateResult - 1]) {
