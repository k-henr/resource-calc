...
import type { ConverterSettingData, ResourceTreeData } from './types';
...
export class ConverterFactory {
	...
	constructor(
		...
		ConverterFactory.getAllPossibleResources(ingredientTreeData, this.possibleIngredients);
		ConverterFactory.getAllPossibleResources(productTreeData, this.possibleProducts);
	}
	...
	private static getAllPossibleResources(data: ResourceTreeData, output: Resource[]): Resource[] {
		switch (data.type) {
			case 'RESOURCE':
			...
			case 'TAG':
				if (!data.tagName) throw new GraphError('A TAG node is missing its tagName attribute!');
				const resources = getResourcesWithTags(data.tagName);
				for (const [, r] of resources) output.push(r);
				return output;
			case 'ENTANGLED_OR':
				data.resources.map(([, r]) => this.getAllPossibleResources(r, output));
				return output;
			case 'BRANCH':
				data.branches.map(([, r]) => this.getAllPossibleResources(r, output));
				return output;
		}
	}
}
