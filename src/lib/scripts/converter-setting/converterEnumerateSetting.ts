...
import type { SettingsTreeEnumerateInput, SettingsTreeInputNode, SettingsTreeNode } from '../types';
...
export class ConverterEnumerateSetting extends ConverterSetting {
	...
	constructor(
		...
		const [settingEl, , select] = ConverterSetting.makeSelectElement(name, onchange);
		...
		for (const optionName of options) {
			...
			if (defIndex === -1)
				throw new GraphError(`Default option "${defaultOption}" not present on setting "${name}"!`);
	...
	public override chooseBranch(data: SettingsTreeInputNode): SettingsTreeNode {
		...
		if (!Object.hasOwn(node, 'options')) {
			throw new GraphError(`Instance of enumerate setting "${data.name}" lacks option list!`);
		}
		...
		for (const [selector, option] of node.options) {
			...
			const selectorMatches =
				typeof selector === 'string' ? selector === chosen : selector.indexOf(chosen) !== -1;
