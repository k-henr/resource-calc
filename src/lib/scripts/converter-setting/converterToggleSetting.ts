...
import type { SettingsTreeInputNode, SettingsTreeNode, SettingsTreeToggleInput } from '../types';
...
export class ConverterToggleSetting extends ConverterSetting {
	...
	constructor(name: string, defaultValue: boolean, onchange: (e: Event) => void) {
		const [settingEl, , input] = ConverterSetting.makeInputElement(name, '', onchange);
		// Add a toggle box
		input.type = 'checkbox';
	...
	public override chooseBranch(data: SettingsTreeInputNode): SettingsTreeNode {
		...
		if (!Object.hasOwn(node, 'true') || !Object.hasOwn(node, 'false')) {
			throw new GraphError(`A branch is missing from the toggle setting "${data.name}"!`);
		}
