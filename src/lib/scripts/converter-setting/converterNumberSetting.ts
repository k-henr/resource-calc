...
export class ConverterNumberSetting extends ConverterSetting {
	...
	constructor(
		...
		const [settingEl, , input] = ConverterSetting.makeInputElement(name, unit, onchange);
	...
	public override chooseBranch(_: SettingsTreeInputNode): SettingsTreeNode {
		const r = Rational.fromInput(this.inputElement.value, this.inputElement)?.getList() ?? 0;
		return r;
	}
