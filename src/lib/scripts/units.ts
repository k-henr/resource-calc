...
import type { UnitGroup, UnitGroupData } from './types';
/**
...
export function loadUnitGroups(groups: [string, UnitGroupData][], defaultGroup: string) {
	defaultUnitGroup = defaultGroup;
	for (const [name, group] of groups) {
		unitGroups.set(name, {
			default: group.default,
			conversions: group.conversions.map(([name, r]) => [name, Rational.fromData(r)])
		});
	}
}

export function convertUnit(groupName: string, amount: Rational, unit: string): Rational {
	...
	if (!conv) throw new GraphError(`Unit ${unit} can't be found in unit group ${groupName}!`);
 ...
 */
export function populateUnitDropdown(selectEl: HTMLSelectElement, groupName: string) {
	selectEl.innerHTML = '';
