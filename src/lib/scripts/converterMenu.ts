...
import type { ConverterDependency, ConverterIngredient } from './types';
/**
...
export class ConverterMenu extends SubmitMenu {
	...
	protected override onSubmit() {
		...
		this.converterInProgress.converter.addProductsToList(this.converterInProgress.products);
	...
	private resolveConverterDependency() {
		...
		this.dependencyPopup.querySelector<HTMLElement>('#converter-dependency-name')!.innerText =
			dependency.converter.thumbName;
		...
		const dependencySettings = new ConverterSettings(dependency.converter.settings, () => {
		...
		dependencySettings.populateForm(
			this.dependencyPopup.querySelector<HTMLFormElement>('#converter-dependency-settings-form')!
		);
		...
		const submitBtn = this.dependencyPopup.querySelector<HTMLElement>('#submit-depencency')!;
		submitBtn.onclick = () => {
			try {
				if (!this.converterInProgress)
					throw new ProgramError(
				...
				const finalDependencyAmount = dependencySettings.evaluateTree(dependency.amount);
				...
				this.converterInProgress.converter.addDependency(
					new Converter(
						dependencySettings.parseFormattedString(dependencyConFact.displayName),
	...
	private finalizeConverter() {
		...
		const amount = this.getAmountToProduce(
			converter,
			this.submissionForm.querySelector<HTMLInputElement>('input[name=amount]')!
		);
	...
	private getAmountToProduce(converter: Converter, input: HTMLInputElement): Rational | null {
	...
	protected override clearFilters() {
		this.filterForm.querySelector<HTMLInputElement>('input[name=search-string]')!.value = '';
	...
	public override applyCurrentFilters() {
		...
		const miscTag = SubmitMenu.createTagListIfNotExists(tagLists, 'Miscellaneous', null);
		...
		for (const [_, cFact] of converterList) {
			...
			let onclickFn = () => {
				...
				this.converterInProgress.converter.settings.populateForm(this.converterSettingsForm);
