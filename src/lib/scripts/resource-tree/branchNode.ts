...
export class BranchNode implements ResourceTree {
	...
	public constructor(settingName: string, childNodes: [string | string[], ResourceTree][]) {
	...
	public addResourcesToList(
		...
		return branch!.addResourcesToList(output, converterDependencies, settings, multiplier);
	}
	...
	private getBranch(settings: ConverterSettings): ResourceTree {
		...
		if (!setting) throw new GraphError(`Setting "${this.settingName}" not found on converter!`);
		...
		const chosenBranchName = (setting as ConverterEnumerateSetting).getChosenOption();
