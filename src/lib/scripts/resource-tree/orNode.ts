...
export class OrNode extends ResourceTreeBoolNode {
	...
	private static converterSelectTemplate = new Template('converter-select-template');
	// Element for containing an option
	private static converterOptionTemplate = new Template('converter-option-template');
	...
	public createElement(): HTMLElement {
		...
		if (this.chosenOption)
			throw new ProgramError('Tried to create an element for an already-collapsed OR node!');
		...
		for (let i = 0; i < this.options.length; i++) {
			...
			for (const name of optionList) {
				...
				const optionContainer = OrNode.converterOptionTemplate.cloneElement();
		...
		el.querySelector<HTMLElement>('.converter-select-count')!.innerText = String(numOptions);
	...
	public override addResourcesToList(
		...
		if (!this.chosenOption)
			throw new UserError("All OR nodes aren't resolved, please choose an option!");
