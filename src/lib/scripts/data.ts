...
import type { ResourceTree } from './resource-tree/resourceTree';
import type { ConverterData, ResourceData, ResourceTreeData, RationalNumber } from './types';
...
export async function loadAllResources(graphName: string) {
	const res = await fetch(`data/${graphName}/resources.json`);
	if (!res.ok) throw new GraphError("Error during resource loading, resources.json doesn't exist!");
	const json: ResourceData[] = await res.json();

	for (const data of json) {
		const r = new Resource(
			data.displayName,
			`data/${graphName}/${data.displayImage}`,
			data.tags ?? [],
...
export function getResourcesWithFilter(searchString: string = '') {
	...
	for (const [id, r] of list) {
		if (searchString && !r.displayName.toLowerCase().includes(searchString.toLowerCase())) continue;
...
export async function loadAllConverters(graphName: string) {
	const res = await fetch(`data/${graphName}/converters.json`);
	if (!res.ok) throw new GraphError("Error during resource loading, converter.json doesn't exist!");
...
export function resourceTreeDataToClass(
	...
	switch (data.type) {
		case 'RESOURCE':
			return new ResourceNode(getResource(data.id), Rational.fromData(data.amount));

		case 'CONVERTER':
			const conFact = getConverterFactory(data.id);
			if (!conFact) throw new GraphError(`Couldn't find converter factory with id "${data.id}"!`);
			return new ConverterNode(conFact, data.amount, data.amountPreview);

		case 'AND':
			return new AndNode(data.resources.map((c) => resourceTreeDataToClass(converter, c)));
		...
		case 'OR':
			...
			return new OrNode(
				options.map((cData, cIndex) => [String(cIndex), resourceTreeDataToClass(converter, cData)])
			);
		...
		case 'ENTANGLED_OR':
			...
			return new EntangledOrNode(
				converter,
				data.id,
				data.resources.map(([cName, cData]) => [cName, resourceTreeDataToClass(converter, cData)])
			);

		case 'MULTIPLIER':
			return new MultiplierNode(resourceTreeDataToClass(converter, data.resource), data.multiplier);

		case 'BRANCH':
			return new BranchNode(
				data.settingName,
				data.branches.map(([name, branch]) => [name, resourceTreeDataToClass(converter, branch)])
			);
		...
		case 'TAG':
			...
			// create an OR
			if (!data.tagName) throw new ProgramError(`A TAG node is missing its "tagName" attribute!`);
			const resources = getResourcesWithTags(data.tagName);
			const resourceData = resources.map(([id]) => makeResourceFromIdAndAmount(id, data.amount));
...
function makeResourceFromIdAndAmount(id: string, amount: RationalNumber): ResourceTreeData {
...
export function getConverterFactoriesWithFilters(
	...
	for (const [id, c] of list) {
		if (searchString && !c.thumbName.toLowerCase().includes(searchString.toLowerCase())) continue;
