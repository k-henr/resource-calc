...
import type { ConverterSettingData, SettingsTreeInputNode, SettingsTreeNode } from './types';
/**
...
export class ConverterSettings {
	...
	constructor(settings: ConverterSettingData[], onchange: (e: Event) => void) {
		...
		for (const data of settings) {
			const setting = ConverterSettings.makeSettingInstance(data, this.onchange);
	...
	private static makeSettingInstance(
		...
		switch (data.type) {
			...
			case 'ENUMERATE':
				return new ConverterEnumerateSetting(data.name, data.default, data.options, onchange);
		}
	}
	...
	public parseFormattedString(input: string): string {
		// Format the string
		return input.replaceAll(/\{(.*?)\}/gim, (_, inner) => this.parseFormatting(inner));
	}
	...
	public evaluateTree(treeNode: SettingsTreeNode): Rational {
		if (typeof treeNode === 'number' || Array.isArray(treeNode)) return Rational.fromData(treeNode);
		...
		switch (treeNode.type) {
			...
			case 'MUL':
				let p = Rational.one;
				for (const child of treeNode.values) p = p.mul(this.evaluateTree(child));
				return p;

			case 'DIV':
				return this.evaluateTree(treeNode.value1).div(this.evaluateTree(treeNode.value2));

			case 'ADD':
				let s = Rational.zero;
				for (const child of treeNode.values) s = s.add(this.evaluateTree(child));
				return s;

			case 'SUB':
				return this.evaluateTree(treeNode.value1).sub(this.evaluateTree(treeNode.value2));

			case 'POW':
				return this.evaluateTree(treeNode.value1).pow(this.evaluateTree(treeNode.value2));
			...
			default:
				throw new GraphError(`Unknown settings AST node type: ${(treeNode as any).type}!`);
		}
	}
}
