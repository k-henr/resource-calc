...
import { displayErr, GraphError, UserError } from './scripts/errors';
import { Template } from './scripts/template';

export async function run(game: string | undefined) {
	const loadingScreen = document.querySelector('#loading-screen')!;
	const loadingText = loadingScreen.querySelector('p')!;
	try {
		if (!game || game === '') throw new UserError('No slug in URL!');
		console.log(game);
		...
		// Get the config file
		const confRes = await fetch(`/data/${game}/config.json`);
		if (!confRes.ok) {
		...
		// Load data files
		await loadAllResources(game);
		await loadAllConverters(game);
		...
		const addRcMenuWrapper = document.querySelector<HTMLElement>('#add-rc-menu-wrapper')!;
		...
		const rHeader = addRcMenuWrapper.querySelector<HTMLElement>('#add-resource-header')!;
		const rUnitDropdown = document.querySelector<HTMLSelectElement>('select#resource-unit-select')!;
		const rFilter = document.querySelector<HTMLFormElement>('form#resource-filter-form')!;
		const rSubmit = document.querySelector<HTMLFormElement>('form#resource-submission-form')!;
		...
		const cHeader = addRcMenuWrapper.querySelector<HTMLElement>('#add-converter-header')!;
		const cFilter = document.querySelector<HTMLFormElement>('form#converter-filter-form')!;
		const cSettings = document.querySelector<HTMLFormElement>('#converter-settings-form')!;
		const cSubmit = document.querySelector<HTMLFormElement>('form#converter-submission-form')!;
		const cSubmitAmount = document.querySelector<HTMLElement>('#converter-amount-input')!;
		const cFormWrapper = document.querySelector<HTMLElement>('#converter-specific-footer')!;
		const cDepdendencyPopup = document.querySelector<HTMLElement>('#converter-dependency-popup')!;
